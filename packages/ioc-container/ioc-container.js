...
IocContainer = class {
  ...
  service(name, Ctor, {transient, concerns, inject, initializable, destroyable} = {}) {
    ...
    model.instances = [];
    model.transient = !!(transient || Ctor.transient);
    model.service = true;
    model.factory = false;
    model.constant = false;
    model.initializable = !!(initializable || Ctor.initializable);
    model.destroyable = !!(destroyable || Ctor.destroyable);
  ...
  factory(name, fn, {transient, concerns, inject, initializable, destroyable} = {}) {
    ...
    let model = this._models[name] = this._models[name] || {};
    model.instances = [];
    model.transient = !!(transient || fn.transient);
    model.service = false;
    model.factory = true;
    model.constant = false;
    model.initializable = !!(initializable || fn.initializable);
    model.destroyable = !!(destroyable || fn.destroyable);
